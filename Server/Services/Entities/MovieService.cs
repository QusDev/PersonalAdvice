...
namespace Server.Services.Entities
{
    public class MovieService : IMovieService
    {
        ...
        {
            if (await _unitOfWork.Movies.IsExistByTitleAsync(dto.Title))
            {
                return Result<bool>.Fail(Error.Conflict($"Movie with title: {dto.Title} already exists"));
            }
        ...
        public async Task<Result<bool>> UpdateAsync(UpdateMovieDto dto)
        {
            ...
            if (dto.Title != null && movie.Title != dto.Title && await _unitOfWork.Movies.IsExistByTitleAsync(dto.Title))
            {
                return Result<bool>.Fail(Error.Conflict($"Movie with title: {dto.Title} already exists"));
            }
